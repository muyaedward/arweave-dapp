<template>
  <div id="signInHeader" class="w-full">
        ...
        <div class="mb-3">
            <h1 class="text-3xl text-gray-800 font-semibold">Academic publications</h1>
            ...
            <div class="full mb-3">
                <div class="flex flex-row">
                    <div class="relative w-4/12">
                        <select v-on:change="event => subjectChange(event)" required name="sortBySubject" v-model="sortBySubject" :class="{ 'bg-gray-200 appearance-none border rounded rounded-r-none rounded-b-none w-full py-3 px-4 leading-tight focus:outline-none focus:bg-white border-blue-500 text-gray-700 focus:border-blue-700': true }">
                    ...
                    <input v-on:change="event => searchTermChange(event)" type="text" required name="sortBytitle" v-model="sortBytitle" placeholder="Search publication by title" :class="{ 'bg-gray-200 appearance-none border rounded rounded-l-none rounded-b-none w-full py-3 px-4 leading-tight focus:outline-none focus:bg-white border-blue-500 text-gray-700 focus:border-blue-700': true }">
                </div>
            </div>
                    ...
                    <tr v-for="(publication, index) in getAllpapers" :key="index" class="px-4 py-2 flex w-full hover:bg-gray-300 justify-between border-dotted border-b-4 border-gray-600 pb-3">
                        <td class="w-9/12 flex flex-row items-center">
                            ...
                            <div>
                                ...
                                <p><a href="#" class="text-blue-500 hover:underline">{{ publication.arweavetags.kitabusubject }}</a>: Publication id: <a href="#" class="text-blue-500 hover:underline">{{ publication.arweavetags.kitabupublicationid }}</a> </p> <!--  Published on: <span class="text-indigo-700 italic">8th November 2019 02:53 AM</span> -->
                            </div>                            
                        </td>
...
export default {
    ...
    components: {},
    computed: {
        ...mapGetters(['getAllPublications', 'getStatusSuccess', 'getStatusError']),
        getAllpapers (){
            if (this.sortBytitle != '') {
    ...
    methods: {
        ...
        subjectChange: function(event) {
            let aw_values = {
                ...
                s_term: this.sortBytitle
            }
            this.$store.dispatch(GET_PUBLICATIONS_SORTED_REQUEST, aw_values).then((resp) => {
                //console.log('got')
            })
        },
        searchTermChange: function(event) {
            let aw_values = {
                subject: this.sortBySubject,
                country: this.sortBycountry,
                s_term: event.target.value
            }
            this.$store.dispatch(GET_PUBLICATIONS_SORTED_REQUEST, aw_values).then((resp) => {
                //console.log('got')
            })
        ...
        openDonateModal(address){
            this.removeAlert()
            let Awaaddress = Cookies.get('aw_address')
            if (Awaaddress == undefined) {
                this.$scrollTo('#signInHeader')
                this.$store.commit(ADD_STATUS_ERROR_REQUEST, 'You can not donate, Kindly log in')
            } else {
                if (address == Awaaddress) {
                    this.$scrollTo('#signInHeader')
                    this.$store.commit(ADD_STATUS_ERROR_REQUEST, 'You can not donate to yourself')                   
                } else {
                    this.$modal.show('donate', {arAddressPassed: Awaaddress}) 
                }                 
            }
        },
